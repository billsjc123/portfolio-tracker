...
**不要直接改任何 JSON 文件**。所有持仓变更都通过 Codex 对话和仓库脚本完成。
...
直接说“再刷新一次”或“立即更新一下看板”，Codex 触发同样的流程。
...
| `qty` | 持仓数量 |
| `cost` | 标的原币种下、按剩余持仓摊算的净投入成本；卖出回收金额超过原始投入后可以为负数 |
| `costCNY` | 可选：成交日冻结的人民币单位成本；外币交易实际以人民币结算时必须填入，防止历史汇率改变成本基数 |
| `shares` | 基金份额（基金专有） |
...
脚本会把上一快照仍持有、但当前 `holdings` 已删除且没有合格 `closedPositions` 的标的视为账务错误并停止更新；这避免了清仓操作悄悄改写历史累计盈亏。`cost` 采用“剩余持仓净投入成本”口径，允许为负数。例如 2 股总成本 100，以 150 卖出 1 股后，剩余 1 股的净投入成本为 -50。校验器不会把该值当作异常或自动修正。
